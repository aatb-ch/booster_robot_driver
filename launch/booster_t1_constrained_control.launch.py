...
"""
Booster Robot Driver - Constrained Position Control Launch File

This launch file starts the Booster T1 robot driver with constrained position control
using native ROS2 topic communication and the ConstrainedPositionController with
Ruckig trajectory generation for smooth motion planning.

Features:
- Native ROS2 topic communication via /joint_ctrl and /low_state
- Constrained control with velocity, acceleration, and jerk limits
...
- controllers_config_file: Controller configuration YAML file (default: constrained_controllers.yaml)
...
Usage:
  ...
  ros2 launch booster_robot_driver booster_t1_constrained_control.launch.py controllers_config_file:=/path/to/custom_config.yaml
"""
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value="constrained_controllers.yaml",
    ...
    # Controller configuration - using constrained controllers
    ...
    constrained_position_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["constrained_position_controller", "-c", "/controller_manager"],
        output="screen",
    )

    # Delay start of constrained_position_controller after joint_state_broadcaster
    delay_constrained_position_controller_spawner_after_joint_state_broadcaster_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=joint_state_broadcaster_spawner,
                on_exit=[constrained_position_controller_spawner],
            )
        )
    )
